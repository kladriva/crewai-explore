...
export default function MonitoringConsole() {
  ...
  const headersInit = useMemo<HeadersInit | undefined>(() => (apiKey ? { "X-API-Key": apiKey } : undefined), [apiKey]);

  async function fetchSnapshot() {
    const init: RequestInit = headersInit ? { headers: headersInit } : {};
    const r = await fetchWithTimeout(`${apiUrl}/api/snapshot`, init);
    const j = await r.json();
    setSnap({ cpu: j.cpu, mem: j.mem, disk: j.disk });
  }
  async function fetchHistory(metric: "cpu" | "mem" | "disk") {
    const init: RequestInit = headersInit ? { headers: headersInit } : {};
    const r = await fetchWithTimeout(`${apiUrl}/api/history?metric=${metric}&minutes=${minutes}&step=15s`, init);
    const j = await r.json();
    const series = j.series?.[0]?.values || [];
    const pts: Point[] = series.map((p: any[]) => ({ time: tsToTime(p[0]), value: parseFloat(p[1]) }))
  ...
  async function refreshAll() {
    setLoading(true);
    try { await Promise.all([fetchSnapshot(), fetchHistory("cpu"), fetchHistory("mem"), fetchHistory("disk")]); }
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <h1 className="text-5xl font-extrabold tracking-tight text-slate-800 text-center mb-2">VPS Monitoring Console</h1>

      {/* Connexion / contrôles */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <Field label="API URL">
            <input className="border rounded-md p-2 w-full focus:outline-none focus:ring-2 focus:ring-sky-400"
                    value={apiUrl} onChange={e => setApiUrl(e.target.value)} placeholder="http://localhost:8000" />
            </Field>
            <Field label="API Key (optionnel)">
            <input className="border rounded-md p-2 w-full focus:outline-none focus:ring-2 focus:ring-sky-400"
                    value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="X-API-Key" />
            </Field>
            <div className="grid grid-cols-2 gap-3">
            <Field label="Fenêtre">
                <select className="border rounded-md p-2 w-full focus:outline-none focus:ring-2 focus:ring-sky-400"
            ...
            <div className="flex items-end">
                <button
                    onClick={refreshAll}
                    disabled={loading}
                    className="w-full rounded-md px-4 py-2 font-medium bg-emerald-600 text-white hover:bg-emerald-500 active:bg-emerald-700 disabled:bg-emerald-400 disabled:text-white disabled:opacity-100 disabled:cursor-not-allowed shadow-sm transition-colors"
                    >
                    {loading ? "…" : "Rafraîchir"}
                </button>
            </div>
      ...
      <div className="rounded-xl p-4 border border-slate-200 bg-gradient-to-b from-slate-50 to-white shadow-sm">
        <div className="flex flex-col sm:flex-row gap-3 items-end">
          <div className="grow">
            <label className="text-xs text-slate-500 block mb-1">Conteneur à redémarrer (whitelist dans <code>rules.yaml</code>)</label>
            <input className="border rounded-md p-2 w-full focus:outline-none focus:ring-2 focus:ring-rose-400"
                   value={target} onChange={e => setTarget(e.target.value)} placeholder="ex: nginx" />
          </div>
          <button
            onClick={restartContainer}
            disabled={actionBusy}
            className="rounded-md px-4 py-2 font-medium bg-emerald-600 text-white hover:bg-emerald-500 active:bg-emerald-700 disabled:opacity-60 shadow-sm"
          >
      ...
      <p className="text-xs text-center text-slate-500">Données: Prometheus • Actions: API FastAPI (CrewAI) • Refresh auto 15s</p>
    </div>
  );
}
